...
void CVCDemoDlg2::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CVCDemoDlg)
	DDX_Control(pDX, IDC_IN, m_In2);
	DDX_Control(pDX, IDC_OUT, m_Out2);
	DDX_Control(pDX, IDC_NOW_P, m_P_Now2);
	DDX_Control(pDX, IDC_HOME_V, m_Home_V2);
	DDX_Control(pDX, IDC_V_ABS, m_V_Abs2);
	DDX_Control(pDX, IDC_V_REF, m_V_Ref2);
	DDX_Control(pDX, IDC_REF, m_Ref2);
	DDX_Control(pDX, IDC_PORT, m_Port2);
	DDX_Control(pDX, IDC_MAXV, m_Maxv2);
	DDX_Control(pDX, IDC_DEC, m_Dec2);
	DDX_Control(pDX, IDC_ACC, m_Acc2);
	DDX_Control(pDX, IDC_ABS, m_Abs2);
	//}}AFX_DATA_MAP
}
...
BEGIN_MESSAGE_MAP(CVCDemoDlg, CDialog)
	...
	ON_BN_CLICKED(IDC_BTN_ACC, OnBtnAcc2)
...
void CVCDemoDlg::OnBtnAcc2()
{
	// TODO: Add your control notification handler code here
	//设置加速度
	CString sValue;
	m_Acc2.GetWindowText(sValue);
	int iAcc;
	iAcc = atoi(sValue);
	INT32 iResult;
	iResult = MT_Set_Axis_Acc(0, iAcc);
